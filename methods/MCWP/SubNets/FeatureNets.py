...
class BertCoAttention(nn.Module):
    ...
    def forward(self, s1_hidden_states, s2_hidden_states, s2_attention_mask):  # [16,30,768]  [16,230,768]  [16,1,1,230]
        # s2_attention_mask  b*1*1*49
        mixed_query_layer = self.query(s1_hidden_states)  # [16,30,768]
        mixed_key_layer = self.key(s2_hidden_states)  # [16,230,768]
        mixed_value_layer = self.value(s2_hidden_states)  # [16,230,768]

        query_layer = self.transpose_for_scores(mixed_query_layer)  # [16,12,30,64]
        key_layer = self.transpose_for_scores(mixed_key_layer) # [16,12,230,64]
        value_layer = self.transpose_for_scores(mixed_value_layer)  # [16,12,230,64]

        # Take the dot product between "query" and "key" to get the raw attention scores.
        attention_scores = torch.matmul(query_layer, key_layer.transpose(-1, -2))  # [16,12,30,230]
        attention_scores = attention_scores / math.sqrt(self.attention_head_size)  # [16,12,30,230]
        ...
        context_layer = torch.matmul(attention_probs, value_layer)  # [16,12,30,64]
        # context_layer b*12*75*64
        context_layer = context_layer.permute(0, 2, 1, 3).contiguous()
        new_context_layer_shape = context_layer.size()[:-2] + (self.all_head_size,)
        context_layer = context_layer.view(*new_context_layer_shape)  # [16,30,768]
