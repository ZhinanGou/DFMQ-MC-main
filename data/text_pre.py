...
class TextDataset:
    ...
    def _get_bert_feats(self, args, examples, base_attrs):
        ...
        if args.text_backbone.startswith('bert'):
            ...
            tokenizer = BertTokenizer.from_pretrained('/home/jiamengyao/MIntRec-main/bert-base-uncased', do_lower_case=True)

        if args.method == 'mcwp':
            features, cons_features, condition_idx, args.max_cons_seq_length = convert_examples_to_features_mcwp(args, examples, base_attrs, tokenizer)     
...
def convert_examples_to_features_mcwp(args, examples, base_attrs, tokenizer):
    """Loads a data file into a list of `InputBatch`s."""
    
    features, cons_features, condition_idx = [], [], []

    prompt_len = args.prompt_len  # 3
    label_len = args.label_len  # 4
    
    max_seq_length = base_attrs['benchmarks']['max_seq_lengths']['text']  # 30
    prefix = ['MASK'] * prompt_len  # ['MASK', 'MASK', 'MASK']
    label_len = label_len
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        input_ids = tokenizer.convert_tokens_to_ids(tokens)
        cons_inputs_ids = tokenizer.convert_tokens_to_ids(cons_tokens)  # The input_mask and segment_ids of input_ids and cons_inputs_ids are the same.
        input_mask = [1] * len(input_ids)
        segment_ids = [0] * len(tokens)
